...
let reason;if(decision==='CALL')reason='CALL setup: all critical gates passed.';else if(decision==='PUT')reason='PUT setup: all critical gates passed.';else if(!marketOpen)reason='NO TRADE — Market closed. Next session confirmation ka wait karein.';else if(!fresh)reason='NO TRADE — Live market snapshot stale. Fresh data ka wait karein.';else if(newsBlock)reason='NO TRADE — High-risk bearish news is blocking the setup.';else if(waiting.length)reason=`NO TRADE — Confirmation insufficient; waiting for ${waiting.join(', ')}.`;else reason='NO TRADE — Directional confirmation insufficient.';
